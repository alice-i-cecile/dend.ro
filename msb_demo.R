...
ggplot(T_df, aes(x=t, y=T_cv)) + geom_line(alpha=0.5) + facet_grid(model~.) + ylim (c(0,3)) + geom_line(data=true_T, colour="red", alpha=0.7) + theme_bw() + xlab("Year") + ylab("Time effect")
...
ggplot(ratio_df, aes(x=t, y=T_cv)) + geom_line() + facet_grid(model~.) + ylim (c(0,3)) + theme_bw() + xlab("Year") + ylab("Ratio between reconstructed and true time effect")
